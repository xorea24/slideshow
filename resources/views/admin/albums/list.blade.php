                    ...
                    <div class="flex flex-col md:flex-row md:items-end justify-between mb-8 pb-8 border-b border-gray-50 gap-6">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <template x-if="selectedPhotos.length > 0">
                                <form action="{{ route('photos.bulkDelete') }}" method="POST" onsubmit="return confirm('Delete ' + selectedPhotos.length + ' selected photos?')">
                                    @csrf
                                    <template x-for="id in selectedPhotos">
                                        <input type="hidden" name="photo_ids[]" :value="id">
                                    </template>
                                    <button type="submit" class="px-4 py-2.5 bg-red-100 text-red-600 hover:bg-red-600 hover:text-white rounded-xl transition-all shadow-sm text-[10px] font-black uppercase flex items-center gap-2">
                                        <i class="fas fa-trash-alt"></i> Delete (<span x-text="selectedPhotos.length"></span>)
                                    </button>
                                </form>
                            </template>
                            ...
                            <form action="{{ route('albums.destroy', $album->id) }}" method="POST" onsubmit="return confirm('Delete entire album?')">
                                @csrf @method('DELETE')
                                <button type="submit" class="p-2.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-all" title="Delete entire album">
                            ...
                            <div :id="'photo-' + {{ $photo->id }}"
                                ...
                                <div class="relative aspect-video bg-gray-100 overflow-hidden">
                                    ...
                                    <div class="absolute top-3 left-3 flex items-center gap-2 z-[30]">
                                        <input type="checkbox" :value="photoId" x-model="selectedPhotos" class="w-4 h-4 accent-blue-600 cursor-pointer rounded shadow-sm">
    ...
    <div class="relative min-h-screen flex items-center justify-center p-4">
        <div class="max-w-5xl w-full bg-white p-10 rounded-[40px] shadow-2xl relative" @click.stop>
            ...
            <div class="mb-6">
                ...
                <p class="text-gray-400 text-xs font-bold uppercase tracking-widest mt-1">Add one by one or drop multiple files below</p>
            </div>
            ...
            <form @submit.prevent="submitForm($event)" action="{{ route('photos.store') }}" method="POST" enctype="multipart/form-data">
                ...
                {{-- 1. ALBUM SELECTION --}}
                <div class="bg-[#f8fbff] p-5 rounded-[24px] border border-[#eef4ff] mb-6">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div class="space-y-1">
                            ...
                            <select name="album_id" x-model="selectedAlbum" @change="isNewAlbum = (selectedAlbum === 'new')" 
                                    class="w-full px-4 py-2.5 rounded-xl border border-blue-100 text-sm font-bold text-gray-600 bg-white outline-none">
                        ...
                        <template x-if="isNewAlbum">
                            <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-3">
                                <div class="space-y-1">
                                    <label class="block text-[10px] font-black text-blue-900 uppercase tracking-widest ml-1">Album Title</label>
                                    <input type="text" name="new_album_name" placeholder="Enter title..." required class="w-full px-4 py-2.5 rounded-xl border border-blue-100 text-sm font-bold outline-none">
                                </div>
                                <div class="space-y-1">
                                    <label class="block text-[10px] font-black text-blue-900 uppercase tracking-widest ml-1">Description</label>
                                    <input type="text" name="new_album_desc" placeholder="Optional..." class="w-full px-4 py-2.5 rounded-xl border border-blue-100 text-sm font-bold outline-none">
                                </div>
                            </div>
                        </template>
                    </div>
                ...
                {{-- 2. BULK UPLOAD DROPZONE (NEW) --}}
                <div class="mb-6 p-8 border-2 border-dashed border-blue-100 rounded-[30px] bg-blue-50/30 hover:bg-blue-50 transition-colors relative group">
                    <input type="file" multiple accept="image/*" class="absolute inset-0 opacity-0 cursor-pointer" 
                           @change="handleBulkUpload($event)">
                    <div class="text-center">
                        <div class="w-12 h-12 bg-blue-100 rounded-2xl flex items-center justify-center mx-auto mb-3 text-blue-600 group-hover:scale-110 transition-transform">
                            <i class="fas fa-cloud-upload-alt text-xl"></i>
                        </div>
                        <p class="text-sm font-black text-blue-900 uppercase tracking-widest">Bulk Upload</p>
                        <p class="text-xs text-blue-400 font-medium">Click or drag multiple images here to auto-fill rows</p>
                    </div>
                </div>

                {{-- BULK APPLY SETTINGS --}}
                <div class="bg-blue-50/30 p-5 rounded-[24px] border border-blue-100 mb-6" x-show="uploadRows.length > 1" x-transition>
                    <h3 class="text-xs font-black text-blue-900 uppercase tracking-widest mb-3">Quick Apply to All Selected Photos</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                        <input type="text" x-model="bulkName" placeholder="(e.g., Event Name)" class="w-full px-4 py-2.5 rounded-xl border border-blue-100 text-sm font-bold bg-white outline-none">
                        <input type="text" x-model="bulkDesc" placeholder="Description (Optional)" class="w-full px-4 py-2.5 rounded-xl border border-blue-100 text-sm font-medium bg-white outline-none">
                    </div>
                    <button type="button" @click="applyBulkData()" class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">
                        <i class="fas fa-magic mr-2"></i> Apply to Below List
                    </button>
                </div>

                {{-- 3. INDIVIDUAL ROWS --}}
                <div class="space-y-3 max-h-[40vh] overflow-y-auto px-1 mb-8 custom-scrollbar">
                    <template x-for="(row, index) in uploadRows" :key="row.id">
                        <div class="group border border-gray-100 rounded-[24px] p-4 flex flex-col md:flex-row gap-4 relative bg-white hover:border-blue-100 transition-all">
                            <div class="w-24 h-24 bg-gray-50 rounded-xl border-2 border-dashed border-gray-200 flex-shrink-0 relative flex flex-col items-center justify-center overflow-hidden">
                                <template x-if="row.preview">
                                    <img :src="row.preview" class="w-full h-full object-cover">
                                </template>
                                <template x-if="!row.preview">
                                    <i class="far fa-image text-gray-300 text-lg"></i>
                                </template>
                                {{-- We keep the single file input so users can still change one specific photo --}}
                                <input type="file" name="images[]" :required="!row.preview" class="absolute inset-0 opacity-0 cursor-pointer" @change="handleFileChange($event, index)">
                            </div>

                            <div class="flex-1 flex flex-col justify-center gap-3">
                                <input type="text" name="name[]" x-model="row.title" placeholder="Image Title" class="w-full px-5 py-2.5 rounded-xl bg-[#f9fafb] border-none text-sm font-bold outline-none">
                                <input type="text" name="descriptions[]" x-model="row.desc" placeholder="Description (Optional)" class="w-full px-5 py-2.5 rounded-xl bg-[#f9fafb] border-none text-sm font-medium outline-none">
                            </div>

                            <button type="button" @click="removePhotoRow(index)" x-show="uploadRows.length > 1" class="absolute -top-2 -right-2 bg-white shadow-md rounded-full w-7 h-7 text-red-400 border border-gray-100">
                ...
                <div class="flex justify-between items-center">
                    <button type="button" @click="addPhotoRow()" class="flex items-center gap-2 text-blue-600 font-black text-[11px] uppercase tracking-widest hover:text-blue-700">
                        <i class="fas fa-plus-circle"></i> Add Another Row
                    </button>
                    <button type="submit" :disabled="isLoading" class="bg-slate-900 hover:bg-black text-white px-8 py-3.5 rounded-xl font-black text-[11px] uppercase tracking-widest btn-upload-shadow transition-all disabled:opacity-50">
...
function galleryManager() {
    return {
        ...
        // --- BULK UPLOAD CORE LOGIC ---
        handleBulkUpload(event) {
            const files = Array.from(event.target.files);
            if (!files.length) return;

            // Clear the default empty row if it hasn't been used
            if (this.uploadRows.length === 1 && !this.uploadRows[0].preview) {
                this.uploadRows = [];
            }
            ...
            files.forEach(file => {
                ...
                // Create a DataTransfer object to simulate a file input for each row
                const dataTransfer = new DataTransfer();
                dataTransfer.items.add(file);

                const newRow = {
                    id: rowId,
                    title: file.name.split('.').slice(0, -1).join('.'), // Use filename as default title
                    desc: '',
                    preview: URL.createObjectURL(file),
                    fileInput: dataTransfer.files // Store the file reference
                };
                
                this.uploadRows.push(newRow);
            });
            
            // Clear the bulk input so it can be used again
            event.target.value = '';
        },
        ...
        async submitForm(event, albumId = null, photoId = null) {
            this.isLoading = true;
            const form = event.target;
            const formData = new FormData(form);

            // If it's the global upload modal, we need to manually append the files 
            // from our uploadRows because they might have been added via Bulk Drag & Drop
            if (!photoId && !albumId && this.showModal) {
                formData.delete('images[]'); // Clear existing to avoid duplicates
                this.uploadRows.forEach(row => {
                    if (row.fileInput) {
                        formData.append('images[]', row.fileInput[0]);
                    }
                });
            }
            ...
            try {
                ...
                if (response.ok) {
                    // Success: Reload to show new items or updated data
                    window.location.reload();
                } else {
                    const errorData = await response.json();
                    alert('Error: ' + (errorData.message || 'Check your inputs.'));
                }
            } catch (error) {
        ...
        applyBulkData() {
            this.uploadRows.forEach(row => {
                if (this.bulkName.trim() !== '') row.title = this.bulkName;
                if (this.bulkDesc.trim() !== '') row.desc = this.bulkDesc;
            });
            // Optional: clear bulk inputs after applying
            this.bulkName = '';
            this.bulkDesc = '';
        },
        resetUploadRows() { this.uploadRows = [{ id: Date.now(), title: '', desc: '', preview: null, fileInput: null }]; },
        addPhotoRow() { this.uploadRows.push({ id: Date.now(), title: '', desc: '', preview: null, fileInput: null }); },
        removePhotoRow(index) { this.uploadRows.splice(index, 1); if(this.uploadRows.length === 0) this.addPhotoRow(); },
        ...
        nextPage() { if(this.page < this.totalPages) { this.page++; this.updateURL(); window.scrollTo({top: 0, behavior: 'smooth'}); } },
        prevPage() { if(this.page > 1) { this.page--; this.updateURL(); window.scrollTo({top: 0, behavior: 'smooth'}); } },
        updateURL() {
